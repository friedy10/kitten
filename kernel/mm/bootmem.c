...
static unsigned long kmem_size =
    (1024 * 1024 * 64); /* default is first 64 MB */
...
static void __init free_all_bootmem_core(struct bootmem_data *bdata) {
  unsigned long pfn;
  unsigned long vaddr;
  ...
  BUG_ON(kmem_max_idx >
         max_idx); /* kmem region doesn't fit into available bootmem */
 ...
 */
unsigned long __init init_bootmem(unsigned long mapstart_pfn,
                                  unsigned long start_pfn,
                                  unsigned long end_pfn) {
...
void __init free_all_bootmem(void) {
...
static void *__init __alloc_bootmem_nopanic(unsigned long size,
  ...
  list_for_each_entry(
      bdata, &bdata_list,
      list) if ((ptr = __alloc_bootmem_core(bdata, size, align, goal,
                                            0))) return (ptr);
  return NULL;
}
 ...
 */
void *__init alloc_bootmem(unsigned long size) {
 ...
 */
void *__init alloc_bootmem_aligned(unsigned long size, unsigned long align) {
 ...
 */
void __init mem_subsys_init(void) {
  early_printk("mem_subsys_init start\n");
  ...
  kmem_create_zone((unsigned long)__va(bootmem_data.node_boot_start),
                   kmem_size);
  early_printk("kmem_create_zone done\n");
  free_all_bootmem();
  early_printk("free_all_bootmem done\n");
  arch_memsys_init(kmem_size);
  early_printk("arch_memsys_init done\n");
}
