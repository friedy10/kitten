 ...
 * ULONG_MAX.
 */
static vaddr_t calc_end(vaddr_t start, size_t extent) {
 ...
 */
static struct region *find_region(struct aspace *aspace, vaddr_t addr) {
  struct region *rgn;
 ...
 */
static struct region *find_overlapping_region(struct aspace *aspace,
 ...
 */
static struct region *find_smartmap_region(struct aspace *aspace,
                                           id_t src_aspace) {
  struct region *rgn;
 ...
 */
static struct aspace *lookup_and_lock(id_t id) {
 ...
 */
static int lookup_and_lock_two(id_t a, id_t b, struct aspace **aspace_a,
                               struct aspace **aspace_b) {
 ...
 */
int __init aspace_subsys_init(void) {
  int status;

  /* Create a hash table that will be used for quick ID->aspace lookups */
  htable = htable_create(7, /* 2^7 bins in the hash table */
                         offsetof(struct aspace, id),
                         offsetof(struct aspace, ht_link), htable_id_hash,
                         htable_id_key_compare);
  if (!htable)
    panic("Failed to create aspace hash table.");
  early_printk("htable_create done\n");
  ...
  early_printk("aspace_create done\n");
  ...
  early_printk("aspace_acquire done\n");

  arch_aspace_activate(current->aspace);
  early_printk("arch_aspace_activate done\n");
...
static id_t id_inc(id_t id) {
  if (++id > UASPACE_MAX_ID)
    id = UASPACE_MIN_ID;
  return id;
}

static id_t id_alloc_any(void) {
  id_t id;
...
static id_t id_alloc_specific(id_t id) {
...
int aspace_create(id_t id_request, const char *name, id_t *id) {
  ...
  if (id_request == KERNEL_ASPACE_ID) {
    ...
    new_id =
        (id_request == ANY_ID) ? id_alloc_any() : id_alloc_specific(id_request);
    if (new_id == ERROR_ID) {
      status = -EEXIST;
      goto fail_id_alloc;
    }
 ...
 */
struct aspace *aspace_acquire(id_t id) {
  struct aspace *aspace;
  unsigned long irqstate;
...
int aspace_update_cpumask(id_t id, cpumask_t *cpu_mask) {
  struct aspace *aspace;
  unsigned long irqstate;
  int ret = 0;
 ...
 */
void aspace_release(struct aspace *aspace) {
  unsigned long irqstate;
...
int __aspace_find_hole(struct aspace *aspace, vaddr_t start_hint, size_t extent,
                       size_t alignment, vaddr_t *start) {
  struct region *rgn;
  vaddr_t hole;
...
int aspace_find_hole(id_t id, vaddr_t start_hint, size_t extent,
                     size_t alignment, vaddr_t *start) {
  int status;
  struct aspace *aspace;
  unsigned long irqstate;
  ...
  status = __aspace_find_hole(aspace, start_hint, extent, alignment, start);
  if (aspace)
    spin_unlock(&aspace->lock);
  local_irq_restore(irqstate);
  return status;
...
int __aspace_add_region(struct aspace *aspace, vaddr_t start, size_t extent,
...
int aspace_add_region(id_t id, vaddr_t start, size_t extent, vmflags_t flags,
                      vmpagesize_t pagesz, const char *name) {
  int status;
  struct aspace *aspace;
  unsigned long irqstate;
...
int __aspace_del_region(struct aspace *aspace, vaddr_t start, size_t extent) {
  int status;
  struct region *rgn;
  vaddr_t end = calc_end(start, extent);
...
int __aspace_map_pmem(struct aspace *aspace, paddr_t pmem, vaddr_t start,
                      size_t extent) {
  int status;
  struct region *rgn;
  ...
  while (extent) {
    ...
    rgn = find_region(aspace, start);
    if (!rgn) {
      printk(KERN_WARNING "Failed to find region covering addr=0x%lx.\n",
             start);
      return -EINVAL;
    }
    ...
    if ((rgn->flags & VM_KERNEL) || (rgn->flags & VM_SMARTMAP)) {
      printk(KERN_WARNING "Trying to map memory to protected region.\n");
      return -EINVAL;
    }

    /* addresses must be aligned to region's page size */
    if ((start & (rgn->pagesz - 1)) || (pmem & (rgn->pagesz - 1))) {
      printk(KERN_WARNING "Misalignment "
    ...
    while (extent && (start < rgn->end)) {
      ...
      status =
          arch_aspace_map_page(aspace, start, pmem, rgn->flags, rgn->pagesz);
      if (status)
        return status;
...
int aspace_map_pmem(id_t id, paddr_t pmem, vaddr_t start, size_t extent) {
  int status;
  struct aspace *aspace;
  unsigned long irqstate;
...
int __aspace_unmap_pmem(struct aspace *aspace, vaddr_t start, size_t extent) {
  struct region *rgn;
  ...
  while (extent) {
    ...
    if (start & (rgn->pagesz - 1)) {
      printk(KERN_WARNING "Start address misalignment (start=0x%lx, "
                          "extent=0x%lx, pagesz=0x%lx).\n",
    ...
    while (extent && (start < rgn->end)) {
      ...
      arch_aspace_unmap_page(aspace, start, rgn->pagesz);
...
int aspace_unmap_pmem(id_t id, vaddr_t start, size_t extent) {
  int status;
  struct aspace *aspace;
  unsigned long irqstate;
...
int __aspace_smartmap(struct aspace *src, struct aspace *dst, vaddr_t start,
                      size_t extent) {
  int status;
  vaddr_t end = start + extent;
  char name[16];
  ...
  if ((status = __aspace_add_region(dst, start, extent, VM_SMARTMAP, PAGE_SIZE,
                                    name)))
    return status;
...
int aspace_smartmap(id_t src, id_t dst, vaddr_t start, size_t extent) {
  int status;
...
int __aspace_unsmartmap(struct aspace *src, struct aspace *dst) {
  struct region *rgn;
  size_t extent;
...
int __aspace_virt_to_phys(struct aspace *aspace, vaddr_t vaddr,
                          paddr_t *paddr) {
  if (!aspace)
    return -EINVAL;
...
int __aspace_lookup_mapping(struct aspace *aspace, vaddr_t vaddr,
...
int aspace_lookup_mapping(id_t id, vaddr_t vaddr, aspace_mapping_t *mapping) {
  int status;
  struct aspace *aspace;
  unsigned long irqstate;
...
int aspace_wait4_child_exit(id_t child_id, bool block, id_t *exit_id,
                            int *exit_status) {
...
int aspace_dump2console(id_t id) {
  struct aspace *aspace;
  struct region *rgn;
  unsigned long irqstate;
  ...
  list_for_each_entry(rgn, &aspace->region_list, link) {
    printk(KERN_DEBUG "    [0x%016lx, 0x%016lx%c %s\n", rgn->start, rgn->end,
           (rgn->end == ULONG_MAX) ? ']' : ')', rgn->name);
  }
...
int aspace_set_rank(id_t id, id_t rank) {
  struct aspace *aspace;
  unsigned long irqstate;
...
int aspace_get_rank(id_t id, id_t *rank) {
  struct aspace *aspace;
  unsigned long irqstate;
...
int aspace_update_hio_syscall_mask(id_t id, syscall_mask_t *syscall_mask) {
  struct aspace *aspace;
  unsigned long irqstate;
